...
int8_t read_user_table_valid_flag()
{
    // KEY_USER_TABLE_VALID
    int8_t value = -1;
    esp_err_t err = nvs_get_i8(app_nvs_handle, keys_lookup[KEY_USER_TABLE_VALID], &value);
    if (err == ESP_OK)
    {
        ESP_LOGI("NVS", "Valid flag value is: %d", (int)value);
    }
    else
    {
        ESP_LOGE("NVS", "Error reading Valid flag value ");
    }
...
{
    // KEY_USER_TABLE_VALID
    esp_err_t err = (nvs_set_i8(app_nvs_handle, keys_lookup[KEY_USER_TABLE_VALID], value));
...
int8_t write_user_table()
{
    ...
    {
        memcpy(buffer + i * size, user_list[i], size);
    }
...
int8_t load_user_table()
{
    ...
    uint32_t size = sizeof(SmartSocketUser);
    uint32_t lsz = 0;
    esp_err_t err = nvs_get_blob(app_nvs_handle, keys_lookup[KEY_USER_TABLE_LIST], buffer, &lsz);
    if (err == ESP_OK)
    {
        for (int i = 0; i < MAX_USER_ENRIES; i++)
        {
            memcpy(user_list[i], buffer+i*size, size);
            user++;
        }
...
int init_nvsflash(nvs_handle_t *nvs_handle_param)
{
    ...
    err = nvs_open("nvs", NVS_READWRITE, &app_nvs_handle);
    if (err != ESP_OK)
    {
    ...
    else
    {
        ...
        {
            printf("Initalizing NVS first time...\n");
            init_user_list();
...
void save_device_info_to_nvs(SmartSocketDevice *dev, char *name)
{
    ...
    // KEY_DEV_ID,
    // KEY_DEV_GID
    esp_err_t err = (nvs_set_u64(app_nvs_handle, keys_lookup[KEY_DEV_ID], dev->mac));
    err = (nvs_set_u64(app_nvs_handle, keys_lookup[KEY_DEV_GID], dev->group_id));
    if (name)
    {
...
void retrive_device_info_from_nvs(SmartSocketDevice *dev, char *name)
{
    ...
    // KEY_DEV_ID,
    // KEY_DEV_GID
    esp_err_t err = (nvs_get_u64(app_nvs_handle, keys_lookup[KEY_DEV_ID], &dev->mac));
    err = (nvs_get_u64(app_nvs_handle, keys_lookup[KEY_DEV_GID], &dev->group_id));
